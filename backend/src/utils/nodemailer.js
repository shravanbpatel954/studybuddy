...
async function sendResetEmail(to, resetUrl){
    try {
        ...
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
            console.warn('⚠️ Email credentials not configured (SMTP_USER or SMTP_PASS missing)');
            console.warn('Reset email would be sent to:', to);
            // In development, just log and return success to avoid breaking the flow
            if (process.env.NODE_ENV !== 'production') {
                console.log('📧 Development mode: Email sending skipped');
                return { success: true, message: 'Development mode: Email not actually sent' };
            }

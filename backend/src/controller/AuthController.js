...
exports.ForgotPassword = async (req, res) => {
    try {
        ...
        // send email - include token in email body for security (not just URL)
        const resetUrl = `${process.env.FRONTEND_URL}/reset-password?token=${token}&email=${encodeURIComponent(email)}`;
        
        // Also include token in email body for manual copy-paste (more secure)
        const resetTokenForEmail = token; // Token to include in email body

        try {
            await sendResetEmail(email, resetUrl, { token: resetTokenForEmail });

...
// Initialize Gemini with QPP API key
...
if (!QPP_API_KEY) {
  ...
  const allQppKeys = Object.keys(process.env).filter(k => k.toUpperCase().includes('QPP'));
  if (allQppKeys.length > 0) {
    console.error('Found QPP-related env vars (but not matching expected format):', allQppKeys);
    console.error('Please ensure your .env file has: QPP_data=YOUR_API_KEY');
  } else {
    console.error('No QPP-related environment variables found at all');
    console.error('Please add to your .env file: QPP_data=AIzaSyAXETZdqXsxQ5owkEfmkGmYKqtz3GHdMXI');
  }
} else {
  console.log('✅ QPP API key loaded successfully (length:', QPP_API_KEY.length, ')');
}

const genAI = QPP_API_KEY ? new GoogleGenerativeAI(QPP_API_KEY) : null;
    ...
    try {
      if (!model) {
        return res.status(500).json({
          success: false,
          error: 'QPP API key not configured. Please set QPP_data in environment variables.'
        });
      }
      ...
      try {
        ...
        for await (const chunk of result.stream) {
          ...
          // Send chunks when buffer reaches a certain size or on newline
          if (buffer.length > 50 || chunkText.includes('\n')) {
  ...
  static async testConnection(req, res) {
    try {
      if (!model) {
        return res.status(500).json({
          success: false,
          error: 'QPP API key not configured'
        });
      }

      const result = await model.generateContent('Say "QPP service is working!"');
